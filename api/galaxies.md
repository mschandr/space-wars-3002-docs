...
## Galaxy Generation Reference

_Migrated from legacy `API_GALAXY_CREATION.md`._

### Expected Generation Times

| Tier | Expected Time | Approximate POIs |
|------|---------------|------------------|
| small | ~6–8 s | ~4,000 |
| medium | ~15–20 s | ~12,000 |
| large | ~25–35 s | ~20,000 |
| massive | ~35–45 s | ~10,000* |

\*Massive tier has fewer POIs due to deferred moon generation.

For larger galaxies, set an appropriate client-side timeout (60–120 seconds):
...
### Size Tier Details (Core vs Outer Stars)
...
The `massive` tier is not returned by `GET /api/galaxies/size-tiers` but can be used directly in creation requests.

### NPC Parameters

NPC count and difficulty are automatically configured by size tier (see Size Tier Specifications above). The creation request also supports explicit overrides:

| Parameter | Type | Description |
|-----------|------|-------------|
| `npc_count` | integer | Number of NPCs for single_player/mixed (default: auto by tier) |
| `npc_difficulty` | string | NPC difficulty: `easy`, `medium`, `hard`, `expert` |
...
- Rich mineral deposits (2× richness)
...
- **Precursor Ship**: Derelict vessel placed in outer region with valuable technology rewards

### Generator Pipeline Breakdown

The creation response includes per-generator timing metrics:

| Generator | Purpose |
|-----------|---------|
| `star_field` | Core + outer star placement |
| `planetary_systems` | Planets, moons, asteroid belts |
| `warp_gate_network` | Core (active) + outer (dormant) gate pairs |
| `mineral_deposits` | Mineral assignment to mineable bodies |
| `defense_network` | Orbital defenses for core systems |
| `trading_infrastructure` | Trading hub creation |
| `precursor_content` | Mirror gate + precursor ship placement |
