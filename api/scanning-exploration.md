...
_Migrated from legacy `API_SCANNING.md`._
...
**Star Charts vs Scanning** — these are separate systems:
...
A player might have a star chart (sees system on map) but never scanned it (no details). Flow:
...
**Travel Auto-Scan**: When traveling to a system, it's automatically scanned at the player's current sensor level. The travel response includes a `scan` object with level, discoveries, and whether more can be revealed.
...
### Baseline Scan Levels by Region
